...
export interface MseDictationResult {
  text: string;
  hasCritical: boolean;
}

/** Ordered list of MSE builder nodes — drives the order of domains in the output. */
export const MSE_BUILDER_DOMAINS: { nodeId: string; label: string }[] = [
...
function containsCritical(phrases: string[]): boolean {
  for (const phrase of phrases) {
    const lower = phrase.toLowerCase();
    for (const critical of CRITICAL_PHRASES) {
      if (lower.includes(critical)) return true;
    }
  }
  return false;
}
 ...
 */
export function buildMseDictation(
  ...
  for (const domain of MSE_BUILDER_DOMAINS) {
    ...
    if (containsCritical(phrases)) hasCritical = true;
