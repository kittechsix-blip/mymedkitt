...
let nodeTreeMap: Record<string, string> | null = null;
...
let currentTreeNodeIds: Set<string> | null = null;
 ...
 */
export function setCurrentTreeId(treeId: string | null): void {
...
async function loadNodeTreeMap(): Promise<Record<string, string> | null> {
  if (nodeTreeMap) return nodeTreeMap;
  ...
  loadPromise = (async () => {
    try {
      ...
      const data = (await res.json()) as NodeTreeMapFile;
 ...
 */
export async function resolveNodeTreeId(nodeId: string): Promise<string | null> {
  ...
  if (currentTreeId) {
    if (currentTreeNodeIds === null) {
      currentTreeNodeIds = new Set(
        Object.keys(map).filter((id) => map[id] === currentTreeId)
      );
    }
    if (currentTreeNodeIds.has(nodeId)) return currentTreeId;
  }
