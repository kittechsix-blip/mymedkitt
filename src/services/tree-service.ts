 ...
 *  Delegates to the full hardcoded fallback — dynamic import pulls the whole
 *  module either way, so a parallel map of per-tree imports was pure duplication. */
async function loadCriticalActionsOnly(treeId: string): Promise<readonly CriticalAction[] | undefined> {
  const config = await loadHardcodedFallback(treeId);
  return config?.criticalActions;
}
...
async function loadHardcodedFallback(treeId: string): Promise<TreeConfig | null> {
  const TREE_IMPORTS: Record<string, () => Promise<TreeConfig>> = {
    ...
    'dysuria-hub': async () => {
      const m = await import('../data/trees/dysuria-hub.js');
      return { nodes: m.DYSURIA_HUB_NODES, entryNodeId: 'dys-start', categoryId: 'emergency-medicine', moduleLabels: m.DYSURIA_HUB_MODULE_LABELS, citations: m.DYSURIA_HUB_CITATIONS, criticalActions: m.DYSURIA_HUB_CRITICAL_ACTIONS, type: 'hub' };
    },
    'painless-scrotal-swelling-hub': async () => {
    ...
    'unknown-ingestion-hub': async () => {
      const m = await import('../data/trees/unknown-ingestion-hub.js');
      return { nodes: m.UNKNOWN_INGESTION_HUB_NODES, entryNodeId: 'ui-start', categoryId: 'toxicology', moduleLabels: m.UNKNOWN_INGESTION_HUB_MODULE_LABELS, citations: m.UNKNOWN_INGESTION_HUB_CITATIONS, criticalActions: m.UNKNOWN_INGESTION_HUB_CRITICAL_ACTIONS, type: 'hub' };
    },
    ...
    'ischemic-limb': async () => {
      const m = await import('../data/trees/ischemic-limb.js');
      return { nodes: m.ISCHEMIC_LIMB_NODES, entryNodeId: 'il-start', categoryId: 'emergency-medicine', moduleLabels: m.ISCHEMIC_LIMB_MODULE_LABELS, citations: m.ISCHEMIC_LIMB_CITATIONS, criticalActions: m.ISCHEMIC_LIMB_CRITICAL_ACTIONS };
    },
    'neck-pain-hub': async () => {
    ...
    'peds-dka': async () => {
      const m = await import('../data/trees/peds-dka.js');
      return { nodes: m.PEDS_DKA_NODES, entryNodeId: 'peds-dka-start', categoryId: 'pediatrics', moduleLabels: m.PEDS_DKA_MODULE_LABELS, citations: m.PEDS_DKA_CITATIONS, criticalActions: m.PEDS_DKA_CRITICAL_ACTIONS };
    },
    ...
    'sedation-options': async () => {
      const m = await import('../data/trees/sedation-options.js');
      return { nodes: m.SEDATION_OPTIONS_NODES, entryNodeId: 'sedopt-start', categoryId: 'pharmacist', moduleLabels: m.SEDATION_OPTIONS_MODULE_LABELS, citations: m.SEDATION_OPTIONS_CITATIONS, criticalActions: m.SEDATION_OPTIONS_CRITICAL_ACTIONS };
    },
    ...
    'fingertip-infections': async () => {
      const m = await import('../data/trees/fingertip-infections.js');
      return { nodes: m.FTI_NODES, entryNodeId: 'fti-start', categoryId: 'orthopedics', moduleLabels: m.FTI_MODULE_LABELS, citations: m.FTI_CITATIONS, criticalActions: m.FTI_CRITICAL_ACTIONS };
    },
    ...
    'emergency-contraception': async () => {
      const m = await import('../data/trees/emergency-contraception.js');
      return { nodes: m.EMERGENCY_CONTRACEPTION_NODES, entryNodeId: 'ec-start', categoryId: 'ob-gyn', moduleLabels: m.EMERGENCY_CONTRACEPTION_MODULE_LABELS, citations: m.EMERGENCY_CONTRACEPTION_CITATIONS, criticalActions: m.EMERGENCY_CONTRACEPTION_CRITICAL_ACTIONS };
    },
    'ohss': async () => {
    ...
    'teg': async () => {
      const m = await import('../data/trees/teg.js');
      return { nodes: m.TEG_NODES, entryNodeId: 'teg-start', categoryId: 'heme-onc', moduleLabels: m.TEG_MODULE_LABELS, citations: m.TEG_CITATIONS, criticalActions: m.TEG_CRITICAL_ACTIONS };
    },
...
export async function getTreeConfig(treeId: string): Promise<TreeConfig | null> {
  ...
  if (isTreeHidden(treeId)) return null;
