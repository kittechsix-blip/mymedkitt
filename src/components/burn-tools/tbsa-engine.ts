...
export type BodyMode = 'adult' | 'peds';
export type BodyView = 'front' | 'back';
export const WIDTH = 360;
export const HEIGHT = 620;
export interface BodyModel { map: Int16Array; ids: string[]; areas: number[] }
const models = new Map<string, BodyModel>();
export function percentages(mode: BodyMode, age: number): Record<string, number> {
  return mode === 'adult' ? TBSA_ADULT_PCT : buildPedsPct(age);
}
export function bodyModel(mode: BodyMode, view: BodyView): BodyModel {
  const key = `${mode}-${view}`;
  ...
  ids.forEach((id, r) => {
    ...
    ctx.save(); ctx.translate(60, 16); ctx.fill(new Path2D(paths[id])); ctx.restore();
    const data = ctx.getImageData(0, 0, WIDTH, HEIGHT).data;
    for (let i = 0; i < map.length; i++) if (data[4 * i + 3] >= 128 && map[i] < 0) { map[i] = r; areas[r]++; }
    if (!areas[r]) throw new Error(`Unpaintable body region: ${id}`);
  });
  const result = { map, ids, areas }; models.set(key, result); return result;
}
function priority(id: string): number { return id === 'perineum' ? 0 : id.includes('neck') ? 1 : id.includes('buttock') ? 2 : 3; }
export function totalBurn(model: BodyModel, paint: Uint8Array, pct: Record<string, number>): number {
  const counts = model.ids.map(() => 0);
  for (let i = 0; i < model.map.length; i++) if (paint[i] && model.map[i] >= 0) counts[model.map[i]]++;
...
export function stroke(model: BodyModel, paint: Uint8Array, x1: number, y1: number, x2: number, y2: number, radius: number, erase = false): void {
  ...
  for (let y = top; y <= bottom; y++) for (let x = left; x <= right; x++) {
    const i = y * WIDTH + x; if (model.map[i] < 0) continue;
    const t = length2 ? Math.max(0, Math.min(1, ((x + .5 - x1) * dx + (y + .5 - y1) * dy) / length2)) : 0;
    if ((x + .5 - x1 - t * dx) ** 2 + (y + .5 - y1 - t * dy) ** 2 <= radius * radius) paint[i] = erase ? 0 : 1;
  }
}
export function renderBody(canvas: HTMLCanvasElement, model: BodyModel, paint: Uint8Array): void {
  const ctx = canvas.getContext('2d')!;
  const img = ctx.createImageData(WIDTH, HEIGHT);
  for (let i = 0; i < model.map.length; i++) {
    const r = model.map[i]; if (r < 0) continue;
