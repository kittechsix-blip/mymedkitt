 ...
 */
export function validInputs(weight: number, tbsa: number): boolean {
  return Number.isFinite(weight) && weight > 0 && Number.isFinite(tbsa) && tbsa > 0 && tbsa <= 100;
}
export function ruleOf10(tbsa: number, weight: number): number {
  if (!validInputs(weight, tbsa)) throw new Error('Enter valid weight and TBSA (0–100%).');
  return tbsa * 10 + Math.max(0, Math.floor((weight - 80) / 10)) * 100;
}
export function dellSeton(weight: number, tbsa: number, cumulative: number, crrt: boolean) {
  if (!validInputs(weight, tbsa) || !Number.isFinite(cumulative) || cumulative < 0) throw new Error('Enter valid weight, TBSA and cumulative volume.');
...
export function parkland(weight: number, tbsa: number, hours: number, given: number, multiplier: 3 | 4) {
  if (!validInputs(weight, tbsa) || !Number.isFinite(hours) || hours < 0 || hours >= 24 || !Number.isFinite(given) || given < 0) throw new Error('Enter valid first-24-hour inputs.');
