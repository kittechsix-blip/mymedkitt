...
export function mountFluids(container: HTMLElement, session = burnSession, initialTab: FluidsTab = 'dell-seton', onChange?: (weight: number | null, tbsa: number | null) => void): () => void {
  ...
  function refresh() {
    ...
    if (tab === 'dell-seton') {
      ...
      if (!d.tier) { result.innerHTML = '<h3>Below this protocol’s ≥20% TBSA entry threshold</h3><p>No Dell Seton formula rate calculated. Assess fluid needs and burn-center referral individually.</p>'; return; }
