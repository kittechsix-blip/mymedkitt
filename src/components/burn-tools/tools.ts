import { bodyModel, HEIGHT, WIDTH, percentages, renderBody, stroke, totalBurn, type BodyMode, type BodyView } from './tbsa-engine.js';
import { LUND_BROWDER_AGES } from './tbsa-paths.js';
import { dellSeton, parkland, ruleOf10, validInputs } from './fluids-engine.js';
export type FluidsTab = 'rule-of-10' | 'parkland' | 'dell-seton';
export interface BurnSession {
  mode: BodyMode; age: number; view: BodyView; paint: Record<BodyView, Uint8Array>;
  tbsa: number | null; weight: number | null; manual: boolean;
  fluids: { volume: string; hours: string; crrt: boolean; multiplier: string };
}
export function createBurnSession(): BurnSession {
...
`;
function shell(container: HTMLElement, html: string): HTMLElement {
  container.innerHTML = '';
  const root = document.createElement('section'); root.className = 'bk-tool';
  const style = document.createElement('style'); style.textContent = css; root.append(style);
  const content = document.createElement('div'); content.style.display = 'contents'; content.innerHTML = html; root.append(content); container.append(root); return root;
}
function numeric(input: HTMLInputElement): number { return input.value.trim() === '' ? NaN : Number(input.value); }
function fmt(n: number): string { return n.toLocaleString('en-US', { maximumFractionDigits: 1 }); }
function clean(session: BurnSession) { session.paint.front.fill(0); session.paint.back.fill(0); session.tbsa = null; session.manual = false; }
export function mountTbsa(container: HTMLElement, session = burnSession, onChange?: (value: number | null) => void): () => void {
  ...
  const manual = root.querySelector<HTMLInputElement>('[data-manual]')!;
  const age = root.querySelector<HTMLSelectElement>('[data-age]')!;
  const ageWrap = root.querySelector<HTMLElement>('[data-age-wrap]')!;
  const error = root.querySelector<HTMLElement>('[data-error]')!;
  const history: { front: Uint8Array; back: Uint8Array; tbsa: number | null; manual: boolean }[] = [];
  let erasing = false, active: number | null = null, last: { x: number; y: number } | null = null;
  function snapshot() { history.push({ front: session.paint.front.slice(), back: session.paint.back.slice(), tbsa: session.tbsa, manual: session.manual }); if (history.length > 25) history.shift(); }
  function refresh(calculate = false) {
    ...
    if (calculate) { session.tbsa = Math.round((front + back) * 10) / 10; session.manual = false; }
    ...
    age.value = String(session.age); ageWrap.hidden = session.mode !== 'peds'; ageWrap.style.display = ageWrap.hidden ? 'none' : '';
    root.querySelectorAll<HTMLButtonElement>('[data-mode]').forEach(b => b.setAttribute('aria-pressed', String(b.dataset.mode === session.mode)));
    root.querySelectorAll<HTMLButtonElement>('[data-view]').forEach(b => b.setAttribute('aria-pressed', String(b.dataset.view === session.view)));
    root.querySelector('[data-action=paint]')!.setAttribute('aria-pressed', String(!erasing)); root.querySelector('[data-action=erase]')!.setAttribute('aria-pressed', String(erasing));
    ...
    session.paint[session.view].forEach((v, i) => { if (v && model.map[i] >= 0) counts[model.map[i]]++; });
  ...
  root.querySelectorAll<HTMLButtonElement>('[data-mode]').forEach(b => b.onclick = () => {
    if (active !== null || session.mode === b.dataset.mode) return;
    clean(session); session.mode = b.dataset.mode as BodyMode; session.view = 'front'; history.length = 0; error.textContent = ''; refresh(true);
  });
  root.querySelectorAll<HTMLButtonElement>('[data-view]').forEach(b => b.onclick = () => { if (active !== null) return; session.view = b.dataset.view as BodyView; refresh(); });
  age.onchange = () => { session.age = Number(age.value); refresh(true); };
  root.querySelectorAll<HTMLButtonElement>('[data-action]').forEach(b => b.onclick = () => {
    if (active !== null) return;
    switch (b.dataset.action) {
      case 'paint': erasing = false; break;
      case 'erase': erasing = true; break;
      case 'undo': { const prev = history.pop(); if (prev) { session.paint = { front: prev.front, back: prev.back }; session.tbsa = prev.tbsa; session.manual = prev.manual; if (!prev.manual) { refresh(true); return; } } break; }
      case 'clear': snapshot(); session.paint[session.view].fill(0); refresh(true); return;
      case 'reset': snapshot(); clean(session); refresh(true); return;
    }
    refresh();
  });
  function point(e: PointerEvent) { const r = canvas.getBoundingClientRect(); return { x: (e.clientX - r.left) * WIDTH / r.width, y: (e.clientY - r.top) * HEIGHT / r.height }; }
  function draw(e: PointerEvent) {
    const p = point(e); const from = last ?? p;
    stroke(bodyModel(session.mode, session.view), session.paint[session.view], from.x, from.y, p.x, p.y, Number(root.querySelector<HTMLSelectElement>('[data-brush]')!.value), erasing);
    last = p;
  }
  canvas.onpointerdown = e => { if (active !== null || !e.isPrimary || e.button !== 0) return; e.preventDefault(); active = e.pointerId; snapshot(); last = null; canvas.setPointerCapture(e.pointerId); draw(e); refresh(true); };
  canvas.onpointermove = e => { if (active !== e.pointerId) return; e.preventDefault(); const events = e.getCoalescedEvents?.() ?? []; (events.length ? events : [e]).forEach(draw); refresh(true); };
  const finish = (e: PointerEvent) => { if (e.pointerId !== active) return; if (e.type === 'pointerup') { draw(e); refresh(true); } active = null; last = null; if (canvas.hasPointerCapture(e.pointerId)) canvas.releasePointerCapture(e.pointerId); };
  canvas.onpointerup = finish; canvas.onpointercancel = finish; canvas.onlostpointercapture = finish;
  manual.oninput = () => {
    if (manual.value === '') { session.tbsa = null; session.manual = true; error.textContent = ''; refresh(); return; }
    const value = numeric(manual);
    if (!Number.isFinite(value) || value < 0 || value > 100) { error.textContent = 'Enter TBSA from 0 to 100%.'; session.tbsa = null; onChange?.(null); root.querySelector('[data-total]')!.textContent = 'Invalid TBSA'; return; }
    error.textContent = ''; session.tbsa = value; session.manual = true; root.querySelector('[data-total]')!.textContent = `${fmt(value)}% TBSA`; root.querySelector('[data-subtotal]')!.textContent = 'Manual override active'; onChange?.(value);
  };
  refresh();
  return () => { if (active !== null && canvas.hasPointerCapture(active)) canvas.releasePointerCapture(active); root.remove(); };
}

export function mountFluids(container: HTMLElement, session = burnSession, initialTab: FluidsTab = 'dell-seton', onChange?: (weight: number | null, tbsa: number | null) => void): () => void {
  const root = shell(container, `
  ...
  `);
  let tab = initialTab;
  const weight = root.querySelector<HTMLInputElement>('[data-weight]')!, tbsa = root.querySelector<HTMLInputElement>('[data-tbsa]')!, volume = root.querySelector<HTMLInputElement>('[data-volume]')!, hours = root.querySelector<HTMLInputElement>('[data-hours]')!, crrt = root.querySelector<HTMLInputElement>('[data-crrt]')!, multiplier = root.querySelector<HTMLSelectElement>('[data-multiplier]')!, result = root.querySelector<HTMLElement>('[data-result]')!;
  volume.value = session.fluids.volume; hours.value = session.fluids.hours; crrt.checked = session.fluids.crrt; multiplier.value = session.fluids.multiplier;
  weight.value = session.weight == null ? '' : String(session.weight); tbsa.value = session.tbsa == null ? '' : String(session.tbsa);
  function visible(selector: string, show: boolean) { const el = root.querySelector<HTMLElement>(selector)!; el.hidden = !show; el.style.display = show ? '' : 'none'; }
  function refresh() {
    const w = numeric(weight), t = numeric(tbsa), v = numeric(volume), h = numeric(hours);
    root.querySelectorAll<HTMLButtonElement>('[data-tab]').forEach(b => b.setAttribute('aria-pressed', String(b.dataset.tab === tab)));
    visible('[data-parkland]', tab === 'parkland'); visible('[data-volume-label]', tab !== 'rule-of-10'); visible('[data-crrt-label]', tab === 'dell-seton'); visible('[data-source]', tab === 'dell-seton');
    result.classList.remove('bk-warning');
    if (!validInputs(w, t)) { result.textContent = 'Enter a weight greater than zero and a TBSA greater than 0 and no more than 100%. No rate calculated.'; return; }
    if (tab !== 'rule-of-10' && (!Number.isFinite(v) || v < 0)) { result.textContent = 'Enter the actual nonnegative volume already given. No rate calculated.'; return; }
    if (tab === 'dell-seton') {
      const d = dellSeton(w, t, v, crrt.checked);
      if (!d.tier) { result.innerHTML = '<h3>TBSA &lt;20%: no protocol-driven burn resuscitation</h3><p>No LR or FFP burn-resuscitation rate indicated by this protocol. Assess fluid needs and burn-center referral individually.</p>'; return; }
      ...
      if (d.ceilingReached) result.classList.add('bk-warning');
      ...
      if (!Number.isFinite(h) || h < 0 || h >= 24) { result.textContent = 'This starting formula covers the first 24 hours from time of burn. Enter 0 to less than 24 hours; at or beyond 24 hours reassess with the burn team.'; return; }
      const m = Number(multiplier.value) as 3 | 4, p = parkland(w, t, h, v, m);
  ...
  root.querySelectorAll<HTMLButtonElement>('[data-tab]').forEach(b => b.onclick = () => { tab = b.dataset.tab as FluidsTab; refresh(); });
  weight.oninput = tbsa.oninput = () => { const w = numeric(weight), t = numeric(tbsa); session.weight = Number.isFinite(w) && w > 0 ? w : null; const next = Number.isFinite(t) && t >= 0 && t <= 100 ? t : null; if (next !== session.tbsa) session.manual = true; session.tbsa = next; onChange?.(session.weight, session.tbsa); refresh(); };
  ...
  volume.oninput = hours.oninput = saveFluids; crrt.onchange = multiplier.onchange = saveFluids;
  refresh(); return () => root.remove();
}
