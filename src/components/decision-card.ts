...
import type { DecisionNode, TreatmentRegimen, DrugRegimen, NodeInput } from '../models/types.js';
...
export interface CardOptions {
  ...
  onContinue?: () => void;
  /** Called when an input-node Continue button fires; receives the selected values. */
  onInputContinue?: (values: string[]) => void;
  onStartOver?: () => void;
  onHome?: () => void;
  config?: TreeConfig;
  treeId?: string;
  ...
  /** Live session answers — required for nodes with a summaryHook. */
...
function renderActiveInput(card: HTMLElement, node: DecisionNode, opts: CardOptions): void {
  const title = document.createElement('h2');
  title.className = 'decision-card__title';
  title.textContent = node.title;
  card.appendChild(title);
...
function renderInputField(
  ...
  const rowsByValue = new Map<string, HTMLButtonElement>();
  ...
  for (const opt of input.options) {
    ...
    const setSelected = (value: string, button: HTMLButtonElement, isSelected: boolean): void => {
      if (isSelected) {
        selected.add(value);
    ...
    row.addEventListener('click', () => {
      if (selected.has(opt.value)) {
        ...
        if (opt.exclusiveGroup) {
          for (const peer of input.options ?? []) {
            if (peer.value !== opt.value && peer.exclusiveGroup === opt.exclusiveGroup) {
              ...
              if (peerRow) setSelected(peer.value, peerRow, false);
            }
          }
        }
        ...
        for (const conflictValue of opt.conflictsWith ?? []) {
          ...
          if (conflictRow) setSelected(conflictValue, conflictRow, false);
        }
...
function renderCalcLinks(container: HTMLElement, node: DecisionNode): void {
  ...
  for (const link of node.calculatorLinks) {
    ...
    const route =
      link.kind === 'tree' ? 'tree' : link.kind === 'info' ? 'info' : 'calculator';
    // `node` deep-links into a tree via the existing /tree/:id/node/:nodeId route.
    const href =
      route === 'tree' && link.node
