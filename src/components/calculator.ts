...
const SFSR_CALCULATOR: CalculatorDefinition = {
  id: 'sfsr',
...
const CSRS_CALCULATOR: CalculatorDefinition = {
  id: 'csrs',
...
const BSA_CALCULATOR: CalculatorDefinition = {
  id: 'bsa',
...
const EPI_INFUSION_CALCULATOR: CalculatorDefinition = {
  id: 'epi-infusion',
...
const BURCH_WARTOFSKY_CALCULATOR: CalculatorDefinition = {
  id: 'burch-wartofsky',
...
const STEROID_EQUIVALENCY_CALCULATOR: CalculatorDefinition = {
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    const equivDoses: Record<number, number> = { 1: 20, 2: 25, 3: 5, 4: 4, 5: 4, 6: 0.75 };
    const equivNames: Record<number, string> = { 1: 'Hydrocortisone', 2: 'Cortisone', 3: 'Prednisone/Prednisolone', 4: 'Methylprednisolone', 5: 'Triamcinolone', 6: 'Dexamethasone' };
...
const SYPHILIS_SEROLOGY_CALCULATOR: CalculatorDefinition = {
...
const SAL_TOX_GUIDE_CALCULATOR: CalculatorDefinition = {
  id: 'sal-tox-guide',
...
const QRS_RISK_CALCULATOR: CalculatorDefinition = {
  id: 'qrs-risk',
...
const BICARB_DOSE_CALCULATOR: CalculatorDefinition = {
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    return {
      ...
      description:
        '▸ BOLUS: ' + lowDose + '–' + highDose + ' mEq IV push (~' + lowAmps + '–' + highAmps + ' amps)\n' +
...
const RUMACK_MATTHEW_CALCULATOR: CalculatorDefinition = {
  id: 'rumack-matthew',
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    if (!hours || !apapLevel) return { value: '--', label: 'Enter values', description: 'Enter hours post-ingestion and APAP level', colorVar: '--color-text-muted' };
    ...
    if (hours > 24) return { value: 'Not Applicable', label: 'Beyond nomogram range', description: 'The Rumack-Matthew nomogram is only validated for 4-24 hours post-ingestion. At >24 hours, treat based on clinical status, transaminases, and INR. If any concern, start NAC empirically.', colorVar: '--color-warning' };
...
const NAC_DOSING_CALCULATOR: CalculatorDefinition = {
  id: 'nac-dosing',
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    if (!rawWeight || rawWeight <= 0) return { value: '--', label: 'Enter weight', description: 'Enter patient weight in kg', colorVar: '--color-text-muted' };
...
const KINGS_COLLEGE_CALCULATOR: CalculatorDefinition = {
  id: 'kings-college',
...
const ALT_APAP_PRODUCT_CALCULATOR: CalculatorDefinition = {
  id: 'alt-apap-product',
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    if (!alt && alt !== 0 || !apap && apap !== 0) return { value: '--', label: 'Enter values', description: 'Enter ALT and APAP concentration', colorVar: '--color-text-muted' };
    if (alt === 0 || apap === 0) {
      if (alt === 0 && apap === 0) return { value: '0', label: 'Both values are zero', description: 'If APAP is undetectable and ALT is normal, hepatotoxicity from acetaminophen is very unlikely. Consider other diagnoses if clinical concern persists.', colorVar: '--color-primary' };
...
const MENING_ABX_CALCULATOR: CalculatorDefinition = {
  id: 'mening-abx',
...
const LP_INTERP_CALCULATOR: CalculatorDefinition = {
  id: 'lp-interp',
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    if (gram >= 2) {
      ...
      const organisms: Record<number, string> = {
...
const AUB_TREATMENT_CALCULATOR: CalculatorDefinition = {
  id: 'aub-treatment',
...
const PE_TREATMENT_CALCULATOR: CalculatorDefinition = {
  id: 'pe-treatment',
...
const COWS_CALCULATOR: CalculatorDefinition = {
  id: 'cows',
  title: 'COWS Score',
...
const PAWSS_CALCULATOR: CalculatorDefinition = {
  id: 'pawss',
  title: 'PAWSS',
...
const CIWA_AR_CALCULATOR: CalculatorDefinition = {
  id: 'ciwa-ar',
  title: 'CIWA-Ar',
...
const MIGRAINE_CRITERIA_CALCULATOR: CalculatorDefinition = {
...
const MIGRAINE_TX_ALGO_CALCULATOR: CalculatorDefinition = {
  id: 'migraine-tx-algo',
...
// ---------------------------------------------------------------------------
const DHE_PROTOCOL_CALCULATOR: CalculatorDefinition = {
  id: 'dhe-protocol',
  title: 'DHE Protocol',
  ...
  computeResult: (values) => {
    ...
    if (cad) contraindications.push('CAD/uncontrolled HTN/PVD');
    if (triptan) contraindications.push('Triptan within 24h');
    if (pregnant) contraindications.push('Pregnancy/breastfeeding');
    if (ergot) contraindications.push('Ergot within 24h');
    if (cyp3a4) contraindications.push('Strong CYP3A4 inhibitor');
...
// ---------------------------------------------------------------------------
const SNAKE_SEVERITY_CALCULATOR: CalculatorDefinition = {
  id: 'snake-severity',
...
// ---------------------------------------------------------------------------
const SNAKE_ANTIVENOM_CALCULATOR: CalculatorDefinition = {
  id: 'snake-antivenom',
...
// ---------------------------------------------------------------------------
const SNAKE_RECURRENCE_CALCULATOR: CalculatorDefinition = {
  id: 'snake-recurrence',
  ...
  computeResult: (values) => {
    ...
    if (inr > 3.0) { thresholds.push('INR > 3.0'); needsRetreat = true; }
    if (ptt > 50) { thresholds.push('PTT > 50 sec'); needsRetreat = true; }
    if (platelets < 25) { thresholds.push('Platelets < 25,000'); needsRetreat = true; }
    if (fibrinogen < 50) { thresholds.push('Fibrinogen < 50 mg/dL'); needsRetreat = true; }

    const multiComponent = thresholds.length >= 2;
    if (multiComponent) needsRetreat = true;
...
// ---------------------------------------------------------------------------
const CORAL_SNAKE_CALCULATOR: CalculatorDefinition = {
  id: 'coral-snake',
...
const CRAO_WINDOW_CALCULATOR: CalculatorDefinition = {
  id: 'crao-window',
  title: 'tPA Window',
...
const CRAO_FUNDUS_CALCULATOR: CalculatorDefinition = {
  id: 'crao-fundus',
  ...
  computeResult: (values) => {
    ...
    const findings: string[] = [];
    if (cherryRed) findings.push('Cherry-red spot (90% of CRAO)');
    if (paleRetina) findings.push('Pale/white retina (58%)');
    if (boxcarring) findings.push('Boxcarring/cattle-trucking (19%)');
    if (attenuated) findings.push('Attenuated arteries (32%)');
    if (embolus) findings.push('Visible embolus');
    if (apd) findings.push('APD present (92-98% sensitive)');
...
const CRAO_DISPO_CALCULATOR: CalculatorDefinition = {
  id: 'crao-dispo',
...
• **Vascular Surgery** (CEA consideration if >70%)`;
    ...
    const workup: string[] = [];
    if (!concurrentStroke) workup.push('☐ MRI brain with DWI');
    else workup.push('☑ MRI brain - concurrent infarcts found');

    if (!carotid) workup.push('☐ Carotid ultrasound/MRA');
    else workup.push('☑ Carotid imaging - stenosis found');

    workup.push('☐ Echocardiogram (TTE, consider TEE)');

    if (!afib) workup.push('☐ Telemetry/Holter for AFib');
    else workup.push('☑ AFib identified');
...
const ROX_INDEX_CALCULATOR: CalculatorDefinition = {
  id: 'rox-index',
  title: 'ROX Index',
...
const KOCHER_CRITERIA_CALCULATOR: CalculatorDefinition = {
  id: 'kocher-criteria',
...
const COPD_SEVERITY_CALCULATOR: CalculatorDefinition = {
  id: 'copd-severity',
...
const SYNOVIAL_WBC_CALCULATOR: CalculatorDefinition = {
  id: 'synovial-wbc',
...
const NEWMAN_CRITERIA_CALCULATOR: CalculatorDefinition = {
  id: 'newman-criteria',
...
const ANTHONISEN_CRITERIA_CALCULATOR: CalculatorDefinition = {
...
const BIPAP_TITRATION_CALCULATOR: CalculatorDefinition = {
  id: 'bipap-titration',
...
const CRP_TREND_CALCULATOR: CalculatorDefinition = {
  id: 'crp-trend',
...
const IV_ORAL_TRANSITION_CALCULATOR: CalculatorDefinition = {
...
const GC_ARTHRITIS_CALCULATOR: CalculatorDefinition = {
  id: 'gc-arthritis',
...
const VPS_MALFUNCTION_CRITERIA_CALCULATOR: CalculatorDefinition = {
...
const VPS_PUMP_TEST_CALCULATOR: CalculatorDefinition = {
  id: 'vps-pump-test',
...
const VPS_ICP_MANAGEMENT_CALCULATOR: CalculatorDefinition = {
...
const MEASLES_IMMUNITY_CALCULATOR: CalculatorDefinition = {
  id: 'measles-immunity',
...
const MEASLES_PEP_CALCULATOR: CalculatorDefinition = {
  id: 'measles-pep',
...
const MEASLES_VITAMIN_A_CALCULATOR: CalculatorDefinition = {
...
const EXTENSOR_ZONE_GUIDE_CALCULATOR: CalculatorDefinition = {
...
const EXTENSOR_SUTURE_GUIDE_CALCULATOR: CalculatorDefinition = {
...
const EXTENSOR_SPLINT_GUIDE_CALCULATOR: CalculatorDefinition = {
...
const TRACH_ALGORITHM_CALCULATOR: CalculatorDefinition = {
  id: 'trach-algorithm',
...
const TRACH_TUBE_SIZE_CALCULATOR: CalculatorDefinition = {
  id: 'trach-tube-size',
...
const TRACH_BLEEDING_CALCULATOR: CalculatorDefinition = {
  id: 'trach-bleeding',
...
const DNI_LUDWIG_CRITERIA_CALCULATOR: CalculatorDefinition = {
...
const DNI_AIRWAY_DECISION_CALCULATOR: CalculatorDefinition = {
...
const DNI_EMPIRIC_ABX_CALCULATOR: CalculatorDefinition = {
  id: 'dni-empiric-abx',
...
const HFNC_SETTINGS_CALCULATOR: CalculatorDefinition = {
  id: 'hfnc-settings',
  ...
  computeResult: (values) => {
    ...
    const settings: Record<number, { title: string; flow: string; fio2: string; temp: string; notes: string }> = {
      1: {
...
const HFNC_ESCALATION_CALCULATOR: CalculatorDefinition = {
  id: 'hfnc-escalation',
...
const OXYGEN_SETTINGS_CALCULATOR: CalculatorDefinition = {
  id: 'oxygen-settings',
  ...
  customRender: (container: HTMLElement, onUpdate: (values: Record<string, number>) => void) => {
    ...
    const devices: Record<string, {
      name: string;
      controls: Array<{ id: string; label: string; unit: string; min: number; max: number; step: number; default: number }>;
      fio2Formula?: (v: Record<string, number>) => string;
      notes: string;
    }> = {
      nc: {
        name: 'Nasal Cannula',
...
const MARINE_CREATURE_ID_CALCULATOR: CalculatorDefinition = {
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    const location = values['location'] as number;
    const mechanism = values['mechanism'] as number;
    const wound = values['wound-pattern'] as number;
    const symptoms = values['symptoms'] as number;
...
const MARINE_IRUKANDJI_SEVERITY_CALCULATOR: CalculatorDefinition = {
  ...
  computeResult: (values: Record<string, number>) => {
    let score = 0;
    if (values['doom']) score += 2;
    if (values['severe-pain']) score += 2;
    if (values['hypertension']) score += 2;
    if (values['tachycardia']) score += 1;
    if (values['diaphoresis']) score += 1;
    if (values['nausea']) score += 1;
    if (values['pulm-edema']) score += 3;
    if (values['cardiac-dysfunction']) score += 3;
...
const MARINE_HOT_WATER_CALCULATOR: CalculatorDefinition = {
  id: 'marine-hot-water',
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    const creature = values['creature'] as number;
    const pain = values['pain-level'] as number;
...
const MARINE_ADMISSION_CRITERIA_CALCULATOR: CalculatorDefinition = {
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    if (values['antivenom']) { score += 3; reasons.push('Antivenom administered'); }
    if (values['resp-distress']) { score += 3; reasons.push('Respiratory compromise'); }
    if (values['cardiac']) { score += 3; reasons.push('Cardiac involvement'); }
    if (values['hypotension']) { score += 3; reasons.push('Hemodynamic instability'); }
    if (values['irukandji']) { score += 3; reasons.push('Irukandji syndrome'); }
    if (values['blue-ring']) { score += 3; reasons.push('Potential tetrodotoxin'); }
    if (values['or-needed']) { score += 3; reasons.push('Surgical intervention needed'); }
    if (values['iv-analgesia']) { score += 2; reasons.push('IV analgesia required'); }
    if (values['severe-cramping']) { score += 2; reasons.push('Severe cramping'); }
    if (values['iv-antibiotics']) { score += 2; reasons.push('IV antibiotics needed'); }
...
const BATTERY_RISK_STRATIFICATION_CALCULATOR: CalculatorDefinition = {
...
const NAT_TEN4_FACESP_CALCULATOR: CalculatorDefinition = {
  id: 'nat-ten4-facesp',
  title: 'TEN-4-FACESp',
  ...
  computeResult: (values: Record<string, number>) => {
    ...
    if (values['torso']) { count++; findings.push('Torso bruising'); }
    if (values['ear']) { count++; findings.push('Ear bruising'); }
    if (values['neck']) { count++; findings.push('Neck bruising'); }
    if (values['age4mo']) { count++; findings.push('Bruise in infant ≤4.99 months'); }
    if (values['frenulum']) { count++; findings.push('Frenulum tear'); }
    if (values['jaw']) { count++; findings.push('Angle of jaw bruising'); }
    if (values['cheek']) { count++; findings.push('Cheek bruising'); }
    if (values['eyelid']) { count++; findings.push('Eyelid/periorbital bruising'); }
    if (values['subconj']) { count++; findings.push('Subconjunctival hemorrhage'); }
    if (values['patterned']) { count++; findings.push('Patterned bruising'); }
...
const QTC_CALCULATOR: CalculatorDefinition = {
  id: 'qtc-calculator',
...
// -------------------------------------------------------------------
// QT Drug Checker (customRender — Searchable CredibleMeds Database)
// -------------------------------------------------------------------

interface QTDrug {
  name: string;
  tradeName?: string;
  risk: 'known' | 'possible' | 'conditional';
  drugClass: string;
  note?: string;
}

const QT_DRUG_DATABASE: QTDrug[] = [
  // Antiarrhythmics
...
const QT_DRUG_RISK_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
...
const QT_DRUG_CHECKER_CALCULATOR: CalculatorDefinition = {
  id: 'qt-drug-checker',
  ...
  customRender: (container: HTMLElement, _onUpdate: (values: Record<string, number>) => void) => {
    ...
    function buildDrugItem(drug: QTDrug): HTMLElement {
    ...
    function renderCategories(): void {
      ...
      const categories = new Map<string, QTDrug[]>();
      for (const drug of QT_DRUG_DATABASE) {
        const cls = drug.drugClass;
        if (!categories.has(cls)) categories.set(cls, []);
      ...
      for (const [catName, drugs] of sortedCats) {
        ...
        for (const drug of drugs) content.appendChild(buildDrugItem(drug));
    ...
    function renderSearchResults(query: string): void {
      ...
      const matches = QT_DRUG_DATABASE.filter(d =>
      ...
      for (const drug of matches) resultsContainer.appendChild(buildDrugItem(drug));
    }
...
const CANADIAN_CT_HEAD_CALCULATOR: CalculatorDefinition = {
  id: 'canadian-ct-head',
...
const WELLS_PE_CALCULATOR: CalculatorDefinition = {
  id: 'wells-pe',
...
const PERC_RULE_CALCULATOR: CalculatorDefinition = {
  id: 'perc-rule',
  title: 'PERC Rule',
...
function addScrollIndicator(scrollParent: HTMLElement): void {
