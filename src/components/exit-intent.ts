...
export interface ExitIntentConfig {
  title: string;
  message: string;
  actions: ExitIntentAction[];
  showOnce?: boolean; // Only show once per session
  cooldownMinutes?: number; // Don't show again for X minutes
}

export interface ExitIntentAction {
  id: string;
  label: string;
  icon: string;
 ...
 */
export function initExitIntent(config: ExitIntentConfig): void {
 ...
 */
function handleMouseOut(e: MouseEvent, config: ExitIntentConfig): void {
 ...
 */
function handleExitIntent(config: ExitIntentConfig): void {
  // Throttle - don't show more than once per 5 seconds
  const now = Date.now();
  if (now - lastShownTime < 5000) return;

  // Don't show if modal already open
  if (isModalOpen) return;

  // Check if should show only once per session
  if (config.showOnce && hasShownThisSession) return;
 ...
 */
function showExitIntentModal(config: ExitIntentConfig): void {
  isModalOpen = true;
 ...
 * Track dismissal
 */
function trackDismissal(actionId: string): void {
  ...
  if (typeof (window as any).gtag !== 'undefined') {
    (window as any).gtag('event', 'exit_intent_action', {
      action_id: actionId,
 ...
 */
export function disableExitIntent(): void {
 ...
 */
export function resetExitIntent(): void {
