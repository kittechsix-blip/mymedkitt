...
import type { MseDictationResult } from '../services/mse-dictation.js';
...
function showCopiedToast(message: string): void {
...
async function copyDictation(text: string): Promise<void> {
  try {
...
async function shareDictation(text: string): Promise<void> {
  if (navigator.share) {
    try {
 ...
 */
function renderDictationBlock(container: HTMLElement, result: MseDictationResult): void {
 ...
 */
export function showMseSummaryModal(
