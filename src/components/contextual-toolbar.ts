...
let infographicEl: HTMLElement | null = null;
...
export function renderContextualToolbar(
  ...
  if (globalTabBar) globalTabBar.style.display = 'none';
...
let infographicEscHandler: ((e: KeyboardEvent) => void) | null = null;
...
function handleInfographicLink(e: MouseEvent): void {
  ...
  if (href.startsWith('#/')) {
    ...
    if (linkType === 'drug') {
      ...
      if (slash !== -1) showDrugModal(linkId.slice(0, slash), linkId.slice(slash + 1));
      else showDrugModal(linkId);
...
function openInfographicOverlay(consultId: string): void {
  ...
  overlay.addEventListener('click', e => {
    if (e.target === overlay) closeInfographic();
  });
  ...
  frame.addEventListener('load', () => {
    try {
      ...
      doc.addEventListener('click', handleInfographicLink as EventListener, true);
    } catch {
