...
export function renderLearnRotation(container: HTMLElement, rotationId: string): void {
  container.innerHTML = '';
...
function renderActionMode(page: HTMLElement, rotationId: string): void {
  const rotation = getRotation(rotationId);
  if (!rotation) return;
  ...
  for (const section of rotation.sections) {
    ...
    for (const card of cards) list.appendChild(renderCardLink(rotationId, card));
...
function renderDomainMode(page: HTMLElement, rotationId: string): void {
  const rotation = getRotation(rotationId);
  if (!rotation) return;
  ...
  for (const dom of ADMSEP_DOMAINS) {
    ...
    const list = document.createElement('div');
    list.className = 'learn-section__cards';
    for (const card of sorted) list.appendChild(renderCardLink(rotationId, card));
    sectionEl.appendChild(list);
...
function renderCardLink(rotationId: string, card: StudentCard): HTMLButtonElement {
  const cardBtn = document.createElement('button');
  cardBtn.className = 'learn-card-link';
  if (card.pillar === 'pharmacology') cardBtn.classList.add('learn-card-link--pharm');
  if (card.pillar === 'condition') cardBtn.classList.add('learn-card-link--condition');
  cardBtn.type = 'button';
...
function renderCoverageBar(rotationId: string, accent: string): HTMLElement {
...
function createToolTile(icon: string, title: string, subtitle: string, onClick: () => void): HTMLButtonElement {
  const btn = document.createElement('button');
...
function renderNotFound(container: HTMLElement, rotationId: string): void {
