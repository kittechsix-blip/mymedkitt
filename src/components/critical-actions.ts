...
export interface CriticalActionsOpts {
...
export function createCriticalActionsButton(opts: CriticalActionsOpts): HTMLButtonElement {
...
function openCriticalActionsModal(opts: CriticalActionsOpts): void {
  ...
  overlay.addEventListener('click', (e) => {
    if (e.target === overlay) closeCriticalActionsModal();
  });

  const escHandler = (e: KeyboardEvent) => {
    if (e.key === 'Escape') closeCriticalActionsModal();
  };
