...
export function renderLearnCard(container: HTMLElement, rotationId: string, cardId: string): void {
  container.innerHTML = '';
  ...
  if (hasLink) {
    ...
    openBtn.addEventListener('click', () => {
      if (linkType === 'info') showInfoModal(linkId);
      else if (linkType === 'drill') router.navigate(`/learn/${rotationId}/drill/${linkId}`);
      else router.navigate(`/tree/${linkId}`);
    });
...
function renderField(label: string, body: string, modifier: string): HTMLElement {
...
function renderRichField(label: string, body: string, modifier: string): HTMLElement {
  const wrap = document.createElement('section');
  wrap.className = `learn-field learn-field--${modifier}`;
...
function renderListField(label: string, items: readonly string[], modifier: string, danger = false): HTMLElement {
  const wrap = document.createElement('section');
...
function renderPimpField(label: string, items: PimpQuestion[]): HTMLElement {
  const wrap = document.createElement('section');
...
function renderNotFound(container: HTMLElement, cardId: string): void {
