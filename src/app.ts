 ...
 */
function handleBareNode(params: RouteParams): void {
  ...
  void resolveNodeTreeId(nodeId).then((treeId) => {
    ...
    if (!window.location.hash.includes(`/node/${nodeId}`)) return;
    if (treeId) {
...
function handleTricksSpecialty(params: RouteParams): void {
  removeContextualToolbar();
  hideGlobalTabBar();
  const main = clearMain();
