...
export const CT_DECISION_SUPPORT_CRITICAL_ACTIONS: CriticalAction[] = [
...
export const CT_DECISION_SUPPORT_NODES: DecisionNode[] = [
...

  {
    ...
    body: 'Any of the following? (~0.9% ciTBI risk if present)\n\n1. Any loss of consciousness\n2. History of vomiting\n3. Severe mechanism (MVC with ejection/death of another passenger/rollover, pedestrian or unhelmeted cyclist struck by motorized vehicle, **fall > 5 ft / 1.5 m**, head struck by high-impact object)\n4. Severe headache\n\n⚠️ **Fall threshold differs by age:** > 5 ft for age ≥ 2 years, > 3 ft for age < 2 years. [7]',
    citation: [7],
    options: [
...

  {
    ...
    module: 2,
    title: 'PECARN < 2 Years — Intermediate Criteria',
    body: 'Any of the following? (~0.9% ciTBI risk if present)\n\n1. LOC ≥ 5 seconds\n2. Non-frontal scalp hematoma (occipital, parietal, or temporal)\n3. Severe mechanism (MVC with ejection/death of another passenger/rollover, pedestrian or unhelmeted cyclist struck by motorized vehicle, **fall > 3 ft / 0.9 m**, head struck by high-impact object)\n4. Not acting normally per parent assessment\n\n⚠️ **Fall threshold differs by age:** > 3 ft for age < 2 years, > 5 ft for age ≥ 2 years. [7]',
    citation: [7],
    options: [
      { label: 'Yes — ≥ 1 intermediate criterion', next: 'ct-pecarn-observe', urgency: 'urgent' },
      { label: 'None present', next: 'ct-pecarn-not-indicated' },
...

  {
    ...
    body: 'For alert (GCS 15), stable trauma patients. In the only large head-to-head trial, **CCR outperformed NEXUS** — sensitivity **99.4% vs 90.7%**, specificity **45.1% vs 36.8%** (n=7,438 analyzable, 162 clinically important injuries). [6]\n\n*Caveat:* NEXUS\'s own 34,069-patient validation reported 99.6% sensitivity, and the CCR advantage lost significance (p=0.09) if all 845 indeterminate cases are counted as rule-negative. **ACR Appropriateness Criteria 2024 treats CCR and NEXUS as co-equal gates.** [6,23]\n\n**Step 1: Any HIGH-RISK factor?** (mandates imaging)\n\n• Age ≥ 65 years\n• Dangerous mechanism:\n  – Fall ≥ 3 ft or ≥ 5 stairs\n  – Axial load to head (e.g., diving)\n  – High-speed MVC (> 100 km/h), rollover, ejection\n  – Motorized recreational vehicle accident\n  – Bicycle collision\n• Paresthesias in extremities',
    citation: [5, 6],
    options: [
...

  {
    ...
    body: '**PE ruled out.** No further testing needed.\n\n**Performance:** sensitivity **97.4%** (95% CI 95.8-98.5), specificity 21.9%, false-negative rate **1.0%** — from the 8,138-patient prospective validation [26]. Note this figure applies to the **combination** of low gestalt AND PERC-negative, not PERC alone.\n\n**RCT-level support:** PROPER (JAMA 2018, 14 EDs, n=1,916) showed a PERC strategy was **noninferior** for 3-month thromboembolic events (0.1% vs 0%) while cutting CTPA use from 23% to 13% and ED stay by 36 min. [27]\n\n⚠️ PERC is **not** validated where PE prevalence is high — a European cohort with 21% prevalence found PERC did not safely exclude PE. Keep the <15% gestalt gate. [26]\n\n**Choosing Wisely:** Avoid CTPA in patients with low pre-test probability AND negative PERC. [20]\n\n**Document:** Pre-test probability assessment (low), PERC applied with all 8 criteria met. No further workup indicated.',
...

  {
    ...
    body: 'Obtain **quantitative D-dimer.** For low/moderate pre-test probability.\n\n**Age-adjusted D-dimer** (patients age **≥ 50**): threshold = **age × 10 µg/L FEU** (= ng/mL FEU). ADJUST-PE failure rate 0.3%. 2026 AHA/ACC multisociety PE guideline: **Class 2a, LOE B-R** for low/intermediate probability. [8,24,25]\n\n⚠️ **UNIT TRAP — this is a 2× error waiting to happen.** Age × 10 assumes your lab reports **FEU** (fibrinogen equivalent units; conventional cutoff 500 ng/mL). If your lab reports **DDU** (D-dimer units; conventional cutoff ~230-250 ng/mL), the age-adjusted threshold is **age × 5**. Confirm your assay\'s units before applying. [25]\n\n**YEARS algorithm** (2026 guideline **Class 2a, LOE B-R**) — an accepted alternative: items = clinical signs of DVT, hemoptysis, PE most likely diagnosis. **0 items → cutoff 1000 µg/L FEU. ≥1 item → cutoff 500 µg/L FEU.** Pregnancy-adapted YEARS is explicitly endorsed in pregnancy. [24]',
    citation: [8, 24, 25],
    options: [
...
export const CT_DECISION_SUPPORT_CITATIONS: Citation[] = [
  ...
  { num: 23, text: 'ACR Appropriateness Criteria: Acute Spinal Trauma — 2024 Update. J Am Coll Radiol. 2025. doi:10.1016/j.jacr.2025.02.013. https://acsearch.acr.org/docs/69359/Narrative/' },
  { num: 24, text: '2026 AHA/ACC/ACCP/ACEP/CHEST/SCAI/SHM/SIR/SVM/SVN Guideline for the Evaluation and Management of Acute Pulmonary Embolism in Adults. Circulation. 2026;153(12):e977-e1051. doi:10.1161/CIR.0000000000001415.' },
  { num: 25, text: 'Righini M, et al. Age-adjusted D-dimer cutoff levels to rule out pulmonary embolism: the ADJUST-PE study. JAMA. 2014;311(11):1117-1124. PMID: 24643601.' },
  { num: 26, text: 'Kline JA, Courtney DM, Kabrhel C, et al. Prospective multicenter evaluation of the pulmonary embolism rule-out criteria. J Thromb Haemost. 2008;6(5):772-780. PMID: 18318689.' },
  { num: 27, text: 'Freund Y, Cachanado M, Aubry A, et al. Effect of the Pulmonary Embolism Rule-Out Criteria on subsequent thromboembolic events among low-risk ED patients: the PROPER randomized clinical trial. JAMA. 2018;319(6):559-566. PMID: 29450523.' },
  { num: 28, text: 'Holmes JF, Yen K, Ugalde IT, et al. PECARN prediction rules for CT imaging of children presenting to the emergency department with blunt abdominal or minor head trauma: a multicentre prospective validation study. Lancet Child Adolesc Health. 2024;8(5):339-347. PMID: 38609287.' },
];
