...
export interface ToolbarItem {
  ...
  /**
   * Action types:
   *  - `calculator`: route to a calculator overlay (target = calculator id).
   ...
   * `pinned: true` items render inline regardless of position; everything else past
   * the visible cap (5) is moved into the 🧰 Tools ▾ drawer. No-op when the parent
   * consult is not opted in.
   */
  pinned?: boolean;
}
...
export interface ToolbarConfig {
  ...
  tools: ToolbarItem[];
  /**
   * Opt-in per consult. When `true`, the contextual-toolbar splits items >5 into a
   * 🧰 Tools ▾ bottom-sheet drawer (separate from the ••• Decision Map).
   * Defaults to `false` — existing 40+ toolbars retain current behavior.
   */
...
const TOOLBAR_CONFIGS: Record<string, ToolbarItem[]> = {
  'oncological-emergencies': [
    ...
    { id: 'crs-icans-protocol', label: 'CRS/ICANS', icon: '🧬', action: 'overlay', target: 'crs-icans-protocol' },
    { id: 'ici-algo', label: 'ICI Tox', icon: '🛡️', action: 'overlay', target: 'ici-toxicity-algorithm' },
    ...
    { id: 'tls-calc', label: 'TLS Risk', icon: '⚗️', action: 'calculator', target: 'tls-cairo-bishop' },
    { id: 'hyperca-calc', label: 'HyperCa', icon: '🦴', action: 'calculator', target: 'hyperca-severity' },
    { id: 'crs-calc', label: 'CRS Grade', icon: '🌡️', action: 'calculator', target: 'crs-grade' },
    { id: 'icans-calc', label: 'ICE/ICANS', icon: '🧠', action: 'calculator', target: 'ice-icans' },
    { id: 'mscc-calc', label: 'MSCC Aid', icon: '🦴', action: 'calculator', target: 'mscc-decision' },
  ...
  'procedural-sedation': [
    ...
    { id: 'agents', label: 'Agents', icon: '💉', action: 'overlay', target: 'ps-agents' },
    { id: 'monitor', label: 'Monitor', icon: '📈', action: 'overlay', target: 'ps-monitor' },
    { id: 'stop', label: 'Stop', icon: '🛑', action: 'overlay', target: 'ps-stop' },
  ],
  'joint-arthrocentesis': [
    { id: 'steps', label: 'Steps', icon: '📋', action: 'overlay', target: 'arth-steps' },
    { id: 'contra', label: 'Contra', icon: '⛔', action: 'overlay', target: 'arth-contra' },
    { id: 'tap', label: 'Tap', icon: '💉', action: 'overlay', target: 'arth-tap' },
    { id: 'fluid', label: 'Fluid', icon: '🧪', action: 'overlay', target: 'arth-fluid' },
    { id: 'stop', label: 'Stop', icon: '🛑', action: 'overlay', target: 'arth-stop' },
  ],
  'difficult-vascular-access': [
    { id: 'steps', label: 'Steps', icon: '📋', action: 'overlay', target: 'dva-steps' },
    { id: 'us-piv', label: 'US PIV', icon: '🩻', action: 'overlay', target: 'dva-us-piv' },
  ...
  'paracentesis': [
    ...
    { id: 'us-site', label: 'US Site', icon: '🩻', action: 'overlay', target: 'para-us-site' },
    { id: 'labs', label: 'Labs', icon: '🧪', action: 'overlay', target: 'para-labs' },
  ...
  'potassium-infusion': [
    { id: 'steps', label: 'Steps', icon: '📋', action: 'overlay', target: 'kclinf-steps' },
    { id: 'rates', label: 'Rates', icon: '💧', action: 'overlay', target: 'kclinf-rates' },
    { id: 'contra', label: 'Contra', icon: '⛔', action: 'overlay', target: 'kclinf-contra-tool' },
    { id: 'dka', label: 'DKA', icon: '🧪', action: 'overlay', target: 'kclinf-dka-tool' },
    { id: 'stop', label: 'Stop', icon: '🛑', action: 'overlay', target: 'kclinf-stop-tool' },
  ],
  'heparin-infusion': [
    { id: 'steps', label: 'Steps', icon: '📋', action: 'overlay', target: 'ufhinf-steps' },
    { id: 'vte', label: 'VTE', icon: '🫁', action: 'overlay', target: 'ufhinf-vte-tool' },
    { id: 'acs', label: 'ACS', icon: '❤️', action: 'overlay', target: 'ufhinf-acs-tool' },
    { id: 'monitor', label: 'Monitor', icon: '🧪', action: 'overlay', target: 'ufhinf-monitor-tool' },
    { id: 'stop', label: 'Reverse', icon: '🛑', action: 'overlay', target: 'ufhinf-reversal-tool' },
  ],
  'hypertonic-saline-protocol': [
    { id: 'steps', label: 'Steps', icon: '📋', action: 'overlay', target: 'hts-steps' },
    { id: 'herniation', label: 'Hernia', icon: '🧠', action: 'overlay', target: 'hts-herniation-tool' },
    { id: 'sodium', label: 'Na Goal', icon: '🧪', action: 'overlay', target: 'hts-sodium-tool' },
    { id: 'hyperna', label: 'HyperNa', icon: '⛔', action: 'overlay', target: 'hts-hyperna-tool' },
    { id: 'stop', label: 'Stop', icon: '🛑', action: 'overlay', target: 'hts-stop-tool' },
  ],
  'scape-nitroglycerin': [
    { id: 'steps', label: 'Steps', icon: '📋', action: 'overlay', target: 'scape-steps' },
    { id: 'dose', label: 'Dose', icon: '💉', action: 'overlay', target: 'scape-dose-tool' },
    { id: 'stop', label: 'Contra', icon: '⛔', action: 'overlay', target: 'scape-contra-tool' },
    { id: 'niv', label: 'NIV', icon: '🫁', action: 'overlay', target: 'scape-niv-tool' },
    { id: 'nipride', label: 'Nipride?', icon: '⚠️', action: 'overlay', target: 'scape-nipride-tool' },
  ],
  'thrombolytic-protocols': [
    { id: 'steps', label: 'Steps', icon: '📋', action: 'overlay', target: 'lytics-steps' },
    { id: 'stroke', label: 'Stroke', icon: '🧠', action: 'overlay', target: 'lytics-stroke-tool' },
    { id: 'stemi', label: 'STEMI', icon: '❤️', action: 'overlay', target: 'lytics-stemi-tool' },
  ...
  'vp-shunt': [
    { id: 'steps', label: 'Steps', icon: '📋', action: 'overlay', target: 'vps-steps' },
    { id: 'risk', label: 'Risk', icon: '📊', action: 'calculator', target: 'vps-malfunction-criteria' },
    { id: 'complications', label: 'Complics', icon: '🧩', action: 'overlay', target: 'vps-complications-tool' },
    { id: 'imaging', label: 'U/S+Image', icon: '🧠', action: 'overlay', target: 'vps-imaging-tool' },
    { id: 'infection', label: 'Infect', icon: '🦠', action: 'overlay', target: 'vps-infection-tool' },
    { id: 'icp', label: 'ICP', icon: '🚨', action: 'overlay', target: 'vps-icp-tool' },
    { id: 'tap', label: 'Tap', icon: '💉', action: 'overlay', target: 'vps-tap-tool' },
    { id: 'stop', label: 'Stop', icon: '🛑', action: 'overlay', target: 'vps-stop' },
  ],
  'xylazine-toxicity': [
  ...
  'culture-positive-results-ed': [
    { id: 'sab', label: 'S. aureus', icon: '🩸', action: 'jump', target: 'cpr-sab' },
    { id: 'candidemia', label: 'Candidemia', icon: '🍄', action: 'jump', target: 'cpr-candidemia' },
    { id: 'esbl', label: 'ESBL', icon: '⚠️', action: 'jump', target: 'cpr-esbl' },
    { id: 'asb', label: 'No-Treat ASB', icon: '🚫', action: 'jump', target: 'cpr-asb-noT' },
    { id: 'cauti', label: 'CAUTI', icon: '🩻', action: 'jump', target: 'cpr-cauti' },
    { id: 'stec', label: 'STEC/HUS', icon: '🚨', action: 'jump', target: 'cpr-stec' },
    { id: 'cdiff', label: 'C. diff', icon: '💩', action: 'jump', target: 'cpr-cdiff-severity' },
    { id: 'gas', label: 'GAS', icon: '🧫', action: 'jump', target: 'cpr-throat-treat' },
    { id: 'dfi', label: 'DFI Stage', icon: '🦶', action: 'jump', target: 'cpr-dfi' },
    { id: 'csf', label: 'CSF', icon: '🧠', action: 'jump', target: 'cpr-csf-admit' },
  ],
  'low-back-pain': [
