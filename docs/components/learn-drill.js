...
let session = null;
export function renderLearnDrill(container, rotationId, drillId) {
    container.innerHTML = '';
...
function renderIntro(container, rotationId, drill) {
    container.innerHTML = '';
...
function renderFeedback(target, step, opt) {
    if (!session)
        return;
    target.innerHTML = '';
    ...
    nextBtn.addEventListener('click', () => {
        ...
        if (container)
            renderStep(container, session.drill.rotationId);
    });
...
function renderDebrief(container, rotationId) {
    if (!session)
        return;
    container.innerHTML = '';
    ...
    if (pct >= 90)
        verdict.textContent = 'Honors-level performance.';
    else if (pct >= 75)
        verdict.textContent = 'Solid pass — review the steps you missed.';
    else if (pct >= 60)
        verdict.textContent = 'Working pass — re-do this drill before the OSCE.';
    else
        verdict.textContent = 'Re-attempt and review the feedback page.';
...
function renderNotFound(container, drillId) {
