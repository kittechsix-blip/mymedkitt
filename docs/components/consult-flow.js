...
function renderConsultSearch(container) {
    if (!controller)
        return;
    const wrapper = document.createElement('div');
    wrapper.className = 'consult-search';
    if (searchOpen)
        wrapper.classList.add('consult-search--open');
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        if (searchOpen) {
            const input = wrapper.querySelector('.consult-search__input');
            if (input) {
                input.value = '';
                input.focus();
            }
...
function getMatchContext(text, terms) {
...
function renderResults(matches, resultsEl, container) {
    resultsEl.innerHTML = '';
    if (matches.length === 0) {
