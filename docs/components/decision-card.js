...
// -------------------------------------------------------------------
function renderActiveQuestion(card, node, opts) {
    // Title
...
// Active info card
// -------------------------------------------------------------------
function renderActiveInfo(card, node, opts) {
    const title = document.createElement('h2');
    title.className = 'decision-card__title';
    title.textContent = node.title;
    card.appendChild(title);
...
function renderActiveResult(card, node, opts) {
    ...
    if (node.confidence) {
        ...
        if (node.confidence === 'definitive')
            ...
            badge.classList.add('decision-card__result-badge--recommended');
        else if (node.confidence === 'consider')
            badge.classList.add('decision-card__result-badge--consider');
...
function renderCalcLinks(container, node) {
