 ...
 */
export function renderStickyDosingHeader(container) {
 ...
 */
export function updateStickyDosingHeader(treatment) {
    if (!headerElement)
        return;
    if (!treatment) {
...
function createDosingRow(label, drug, isPrimary) {
...
function copyTreatmentToClipboard(treatment) {
    const lines = [];
    const formatDrug = (drug) => {
        return [
            drug.drug,
            drug.dose,
            drug.route,
 ...
 */
export function clearStickyDosingHeader() {
    if (headerElement) {
        headerElement.classList.add('sticky-dosing-header--hidden');
        headerElement.setAttribute('aria-hidden', 'true');
        headerElement.innerHTML = '';
