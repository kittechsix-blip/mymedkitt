...
const METHB_BLUE_DOSING_CALCULATOR = {
...
const METHB_FUNCTIONAL_HB_CALCULATOR = {
    ...
    computeResult: (values) => {
        ...
        const methb = values['methb'] || 0;
        if (hb <= 0 || methb < 0)
            return { value: '--', label: 'Enter values', description: 'Enter total Hb and MetHb %.', colorVar: '--color-text-muted' };
...
const METHB_G6PD_RISK_CALCULATOR = {
    id: 'methb-g6pd-risk',
...
const AORTIC_ANTI_IMPULSE_CALCULATOR = {
    ...
    computeResult: (values) => {
        ...
        const sbp = values['sbp'] || 0;
        if (hr <= 0 && sbp <= 0)
            return { value: '--', label: 'Enter vitals', description: 'Enter current HR and SBP.', colorVar: '--color-text-muted' };
        let hrStatus = '';
        let sbpStatus = '';
        let colorVar = '--color-primary';
        const targetHR = '60-70 bpm';
        const targetSBP = '100-120 mmHg';
        if (hr > 70) {
            hrStatus = 'HR ' + hr + ' > target. Increase beta-blocker.';
            colorVar = '--color-warning';
        }
        else if (hr >= 60) {
            hrStatus = 'HR ' + hr + ' at target.';
        }
        else if (hr > 0) {
            hrStatus = 'HR ' + hr + ' < target. May decrease beta-blocker if symptomatic bradycardia.';
        }
        if (sbp > 120) {
            sbpStatus = 'SBP ' + sbp + ' > target. Add vasodilator (nicardipine) AFTER HR controlled.';
            colorVar = '--color-warning';
        }
        else if (sbp >= 100) {
            sbpStatus = 'SBP ' + sbp + ' at target.';
        }
        else if (sbp > 0) {
            sbpStatus = 'SBP ' + sbp + ' < target. Assess perfusion. May need pressors if hypotensive dissection.';
            colorVar = '--color-danger';
        }
...
const AORTIC_TYPE_GUIDE_CALCULATOR = {
...
const AAA_PERMISSIVE_HYPOTENSION_CALCULATOR = {
    ...
    computeResult: (values) => {
        ...
        if (sbp <= 0)
            return { value: '--', label: 'Enter SBP', description: 'Enter current systolic BP.', colorVar: '--color-text-muted' };
        let status = '';
        let colorVar = '--color-primary';
        if (sbp > 90) {
...
const HEAT_STROKE_FLUID_CALCULATOR = {
    id: 'fluid-calculator',
...
// -------------------------------------------------------------------
const MCMAHON_RHABDO_CALCULATOR = {
    id: 'mcmahon-rhabdo',
    title: 'McMahon Score',
...
const SHOULDER_RECURRENCE_CALCULATOR = {
...
const REDUCTION_TECHNIQUE_CALCULATOR = {
...
// -------------------------------------------------------------------
const PEDS_SUBMERSION_SEVERITY_CALCULATOR = {
...
// -------------------------------------------------------------------
const PEDS_DROWNING_PROGNOSIS_CALCULATOR = {
...
// -------------------------------------------------------------------
const SUBMERSION_OBSERVATION_CALCULATOR = {
...
// -------------------------------------------------------------------
const BRUGADA_SHANGHAI_CALCULATOR = {
    id: 'brugada-shanghai',
...
// -------------------------------------------------------------------
const BRUGADA_VF_STORM_CALCULATOR = {
    id: 'brugada-vf-storm',
...
// -------------------------------------------------------------------
const BRUGADA_DRUGS_AVOID_CALCULATOR = {
...
// -------------------------------------------------------------------
const HD_EMERGENCY_TRIAGE_CALCULATOR = {
...
// -------------------------------------------------------------------
const HD_HYPERKALEMIA_CALCULATOR = {
    id: 'hd-hyperkalemia',
...
// -------------------------------------------------------------------
const HD_ACCESS_EMERGENCY_CALCULATOR = {
...
// -------------------------------------------------------------------
const HD_DDS_PREVENTION_CALCULATOR = {
...
// =====================================================================
const HUNTER_CRITERIA_CALCULATOR = {
    id: 'hunter-criteria',
...
const CYPROHEPTADINE_DOSE_CALCULATOR = {
...
// =====================================================================
const XYL_WOUND_STAGING_CALCULATOR = {
...
const ATLANTA_SEVERITY_CALCULATOR = {
    id: 'atlanta-severity',
...
const MODIFIED_MARSHALL_CALCULATOR = {
...
const AP_FLUID_RATE_CALCULATOR = {
    id: 'ap-fluid-rate',
...
const INFECTED_NECROSIS_STEP_CALCULATOR = {
...
const CSSRS_SCREEN_CALCULATOR = {
    ...
    computeResult: (values) => {
        ...
        const positives = [];
        if (q1 > 0)
            positives.push('Wish to be dead');
        if (q2 > 0)
            positives.push('Suicidal thoughts');
        if (q3 > 0)
            positives.push('Method considered');
        if (q4 > 0)
            positives.push('Intent to act');
        if (q5 > 0)
            positives.push('Plan with intent');
        if (q6 > 0)
            positives.push('Recent behavior');
        if (maxLevel === 0) {
            return {
...
// Safety Plan Builder
// -------------------------------------------------------------------
const SAFETY_PLAN_BUILDER_CALCULATOR = {
    ...
    computeResult: (values) => {
        ...
        const missing = [];
        if (step1 === 0)
            missing.push('1. Warning Signs');
        if (step2 === 0)
            missing.push('2. Internal Coping');
        if (step3 === 0)
            missing.push('3. Social Contacts');
        if (step4 === 0)
            missing.push('4. Support People');
        if (step5 === 0)
            missing.push('5. Professional Resources');
        if (step6 === 0)
            missing.push('6. Lethal Means Restriction');
        if (completed === 0) {
            return {
                value: '0/6 Steps',
