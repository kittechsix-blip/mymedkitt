...
export function buildSpecialtyGradient(base) {
...
export function getSpecialtyGradient(specialtyId) {
    const colors = getCategoryColors();
    const entry = colors[specialtyId];
    if (!entry)
        return 'linear-gradient(to bottom, #6a6a6a 0%, #4a4a4a 40%, #3a3a3a 60%, #2a2a2a 100%)';
