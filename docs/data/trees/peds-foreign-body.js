...
export const PEDS_FB_CITATIONS = [
    ...
    { num: 12, text: 'Joyner BL Jr, et al. Part 6: Pediatric Basic Life Support: 2025 American Heart Association and American Academy of Pediatrics Guidelines for CPR and ECC. Circulation. 2025 Oct 21;152(16_suppl_2):S424-S447. PMID 41122891. (Supersedes Topjian 2020 Part 4; ≥1yr choking now back blows alternating with abdominal thrusts.)' },
    ...
    { num: 19, text: 'Clinker C, et al. Outcomes of pediatric esophageal button battery ingestion (n=713). J Pediatr Surg. 2026. (Stricture ~3.9%, TEF ~5.2% overall incidence — contrasts with the Litovitz major-outcome distribution.)' },
    { num: 20, text: 'Zoizner-Agar G, Rotsides JM, Shao Q, et al. Diagnostic criteria for foreign body aspiration: clinical, radiographic and bronchoscopic findings (n=5,606). Int J Pediatr Otorhinolaryngol. 2020;139:110442. PMID 33068947. (Classic triad sensitivity 35%, specificity 90%.)' },
    { num: 21, text: 'Truong DT, et al. Predictive value of history and physical examination in pediatric foreign body aspiration. 2023. (History specificity as low as 23.1% in an unselected population.)' },
    { num: 22, text: 'Lee JH. Foreign body ingestion in children. Clin Endosc. 2018;51(2):129-136. (Suggests a 4-5 cm length threshold in infants and young children, extrapolated from NASPGHAN\'s >6 cm.)' },
    { num: 23, text: 'de Benito Sanz M, et al. Efficacy of glucagon for esophageal food impaction: a randomized, double-blind, placebo-controlled trial. Am J Gastroenterol. 2024. PMID 37734342. (23.6% vs 20.6% placebo, p=0.67.)' },
    { num: 24, text: 'Peksa GD, et al. Glucagon for relief of acute esophageal foreign bodies and food impactions: a systematic review and meta-analysis. Pharmacotherapy. 2019;39(4):463-472. PMID 30779190. (OR 0.90, 95% CI 0.69-1.17; increased vomiting.)' },
];
export const PEDS_FB_CRITICAL_ACTIONS = [
    ...
    { text: '2025 AHA/AAP change: child ≥1yr choking = 5 back blows ALTERNATING with 5 abdominal thrusts (not thrusts alone) [12]', nodeId: 'peds-fb-bls' },
...
export const PEDS_FB_NODES = [
    ...
    // ====================================================================
    {
        ...
        body: '**History red flags** [4,8,14]\n- Witnessed choking event — **sensitive but NOT reliably specific.** Sensitivity 76-92%; specificity varies enormously by population (reported as high as ~95% in selected bronchoscopy cohorts but as low as 23% when history alone is used in an unselected ED population). Treat a witnessed event as a trigger to evaluate, not as confirmation. [8,21]\n- Sudden cough or apnea while eating/playing\n- Persistent unilateral wheeze\n- Recurrent same-lobe pneumonia\n\n**Exam findings (any may be absent):**\n- Focal decreased breath sounds (most specific finding)\n- Unilateral wheeze\n- Stridor (extrathoracic/laryngeal FB)\n- Drooling, dysphonia\n\n**Key point:** Roughly **20-40%** of confirmed FBA have a normal exam at presentation. A normal exam plus a normal film still does not clear the airway. [8,14]\n\n**Common aspirated items** [4]\n- Peanuts (most common in <3yr)\n- Other nuts, seeds, popcorn\n- Hot dog, grape, candy\n- Small toy parts, balloon fragments, pen caps\n- Coins (can lodge in airway if very small)',
        citation: [4, 8, 14, 21],
        next: 'peds-fb-airway-imaging',
        summary: 'Witnessed event sens 76-92% but specificity population-dependent (23-95%); focal ↓ BS = most specific finding; 20-40% have normal exam; peanuts most common <3yr',
    },
    {
    ...
    },
    {
        ...
        body: '**Sharp objects** (needles, pins, fish/chicken bones, broken glass, open safety pins): [1,4]\n\n**Esophageal:**\n- Endoscopic removal regardless of symptoms — risk of perforation\n- **Symptomatic → <2 hours**\n- **Asymptomatic → <24 hours**\n\n**Gastric, sharp object:**\n- Endoscopic removal if technically feasible (point-trailing technique)\n- Failed endoscopy + sharp object → close observation + serial imaging\n\n**Long or wide objects — NASPGHAN 2015 thresholds:** [1]\n- **Length >6 cm** (any age) → endoscopic removal; will not round the duodenal sweep\n- **Diameter >25 mm** → endoscopic removal; will not pass the pylorus\n- Practical adjustment for infants and toddlers: a lower threshold (~4-5 cm) is reasonable in a small child, but note this is expert-opinion extrapolation, not the NASPGHAN number. [22]\n\n**Beyond duodenum:**\n- Serial daily X-rays\n- Surgical consult if no progression × 3 days or symptoms develop',
        citation: [1, 4, 22],
        next: 'peds-fb-disposition',
        summary: 'Sharp esoph = endoscopy regardless of sx (<2h sx, <24h asx); NASPGHAN: length >6cm OR diameter >25mm = endoscopy',
    },
    {
        id: 'peds-fb-food',
        type: 'info',
        ...
        body: '**Esophageal food bolus** (uncommon in young children without underlying disease — eosinophilic esophagitis, stricture, web): [1,4]\n\n**Symptomatic, unable to handle secretions:** Endoscopic removal <2 hours\n\n**Symptomatic, handling secretions:** Endoscopic removal <24 hours\n\n**Pharmacologic options (limited evidence in peds):**\n- **[Glucagon](#/drug/glucagon/food-bolus)** 0.5 mg IV in child (1 mg in adolescent) — relaxes LES, in theory may allow passage. **⚠️ The adult evidence no longer supports it:** a 2024 RCT found passage in 23.6% with glucagon vs 20.6% with placebo (p=0.67), and a meta-analysis found no benefit (OR 0.90, 95% CI 0.69-1.17), with more vomiting in the glucagon arm. Pediatric data are essentially absent. **Do not delay endoscopy to try glucagon.** [1,23,24]\n- No role for nifedipine or carbonated beverages in pediatrics\n\n**Post-extraction:**\n- All pediatric food bolus = referral to GI for EGD + biopsy (EoE workup)\n- Strict avoidance of solid food until cleared',
        citation: [1, 4, 23, 24],
        next: 'peds-fb-disposition',
        summary: 'Peds food bolus rare without dz; sx + no secretions = <2h; glucagon no better than placebo in adult RCT — do not delay endoscopy; ALL peds food bolus → GI for EoE workup',
    },
    // ====================================================================
