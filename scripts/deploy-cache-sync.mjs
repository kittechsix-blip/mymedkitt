...
if (!diffOutput.trim()) {
  ...
  if (treeFiles.length > 0) {
    ...
    for (const treeFile of treeFiles) {
      ...
      let treeUpdates = 0;
      const nodeFields = ['type', 'module', 'title', 'body', 'citation', 'options', 'inputs', 'next', 'recommendation', 'treatment', 'confidence', 'images', 'calculatorLinks', 'summary', 'skippable', 'safetyLevel', 'whenToUse', 'pearls', 'evidence'];
...
//    exist on disk and cache.addAll() rejected). Walks docs/, includes app
//    code/data, excludes large clinical images and source archives.
// ---------------------------------------------------------------------------
console.log('\n=== Step 5: Regenerating SW precache asset list ===\n');

{
  const swFile = resolve(projectRoot, 'docs/sw.js');
  const startMarker = '// AUTO-GENERATED ASSETS START';
  const endMarker = '// AUTO-GENERATED ASSETS END';

  // Files to precache: HTML, CSS, JS, JSON across docs/, plus small UI icons
  // under docs/assets (NOT docs/images/ — 142MB clinical images served on-demand
  // via cache-first fetch handler). Skip sw.js itself, .DS_Store, source archives.
  function walkDocs(dir, results = []) {
    for (const entry of readdirSync(dir)) {
      if (entry === '.DS_Store') continue;
      const full = join(dir, entry);
      const rel = relative(docsDir, full);
      // Skip directories that shouldn't be precached
      if (rel === 'images' || rel.startsWith('images' + '/')) continue;
      if (rel === 'assets/icons/source' || rel.startsWith('assets/icons/source/')) continue;
      ...
      if (!/\.(html|css|js|json|png|svg)$/i.test(entry)) continue;
      results.push('./' + rel);
    }
    return results;
  }

  const found = walkDocs(docsDir).sort();
  const assets = ['./', ...found];

  const block = [
    startMarker + ' — do not edit by hand.',
  ...
  if (!re.test(swContent)) {
    console.log('⚠ Could not find AUTO-GENERATED ASSETS markers in docs/sw.js — skipping');
  } else {
    const newSw = swContent.replace(re, block);
    if (!dryRun) {
      writeFileSync(swFile, newSw, 'utf-8');
      console.log(`✅ SW asset list regenerated: ${assets.length} entries`);
    } else {
      console.log(`📝 Would write ${assets.length} entries to docs/sw.js asset list`);
    }
  }
}
